...
class pipeline_tree:
    ...
    def generate_leaf_runs(self, params: pd.DataFrame) -> pd.DataFrame:
        """collect runids for each leaf node"""
    ...
    def calculate_node_scores_return_df(
        ...
        df: dataframe of all runids and stats
        stat: list of stats to calculate
        """
    ...
    def calculate_node_scores_with_source_subset_draw(
        self, params: pd.DataFrame, df=pd.DataFrame, cv=10, subset=0.6
    ):
        ...
        for i in range(cv):
            sample_sources = np.random.choice(
                df_sources, int(len(df_sources) * subset), replace=True
            )
    ...
    def tree_scores(
        self, stats=["coverage", "depth"], node_color=None, internode_function=np.max
    ):
        """ """
        ...
        def tree_weights(node):
            ...
            path_to_leaf_weights.loc[node] = internode_function(td)
    ...
    def get_leaves_df(self):
        pipe_tree_df = self.node_weights.copy()
        #####
        pipe_tree_df_leaves = pipe_tree_df.iloc[self.leaves]
        pipe_tree_df_leaves = pipe_tree_df_leaves.drop(columns=["finished"])
        return pipe_tree_df_leaves
